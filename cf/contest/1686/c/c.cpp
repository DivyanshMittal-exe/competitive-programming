...
void solve()
{
    ...
    for (int i = 0; i < count; i++)
    {
        if(i%2 == 0){
            ...
            cic[i] = values[(count>>1) + (i>>1)];
        }
    }

    values = cic;

    if((values[0] > values[1] && values[0] > values[count-1])||(values[0] < values[1] && values[0] < values[count-1])){
    ...
    if((values[count-1] > values[0] && values[count-1] > values[count-2])||(values[count-1] < values[0] && values[count-1] < values[count-2])){
